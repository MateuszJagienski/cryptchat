...
public class Main {
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException, InvalidKeyException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, InvalidKeySpecException, InvalidAlgorithmParameterException, IOException {
        ...
        User user1 = new User("user1", KeysGenerator.generateRsaKeyPair(4096));
        // Another user connec
        User user2 = new User("user2", KeysGenerator.generateRsaKeyPair(4096));
        ...
        user1Message.setEncryptedText(AesCipher.encrypt(user1Message.getText(), user1.getSecretAesKey()).getBytes());
        System.out.println("enc mess" + BytesToStringConverter.bytesToString(user1Message.getEncryptedText()));
        ...
        var hashedMessage = HashGenerator.hashMessageSHA(user1Message.getText(), "SHA-1", 128);
        byte[] signedHashMessage = RsaAuth.signWithPrivateKey(hashedMessage, user1.getKeyPairRsa().getPrivate());
        user1Message.setMessageSignature(signedHashMessage);

        // user2 receive message and signed hashed message
        // user2 decrypt message with shared key
        var dec = AesCipher.decrypt(BytesToStringConverter.bytesToString(user1Message.getEncryptedText()), user2.getSecretAesKey());
